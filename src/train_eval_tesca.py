...
def train_tesca(args, model, train_data_loader, dev_data_loader):
    ...
    switch_epoch = args['num_epochs'] * args['switch_ratio']
    ...
    for epoch in range(args['num_epochs']):
        ...
        for iteration, train_batch in enumerate(train_data_loader):
            model.train()
            senti_labels, topic_labels = train_batch['senti_labels'], train_batch['topic_labels']
            optimizer.zero_grad()
            # pred_outputs = model(inputs)

            senti_loss, topic_loss, senti_logits, topic_logits, seq_atten, topic_atten = model(**train_batch)

            if model.switch == 0:
                loss = topic_loss
            else:
                loss = senti_loss + args['loss_ratio'] * topic_loss

            loss.backward()
            optimizer.step()
            scheduler.step()
            train_stat.add(senti_logits, senti_labels, topic_logits, topic_labels, loss.item())
            ...
            if (iteration + 1) % args['display_per_batch'] == 0:
                ...
                with torch.no_grad():
                    for dev_batch in dev_data_loader:
                        senti_labels, topic_labels = dev_batch['senti_labels'], dev_batch['topic_labels']
                        senti_loss, topic_loss, senti_logits, topic_logits, seq_atten, topic_atten = model(**dev_batch)
                        ...
                        dev_stat.add(senti_logits, senti_labels, topic_logits, topic_labels, loss.item())
...
def test_tesca(args, model, test_data_loader, class_list):
    ...
    with torch.no_grad():
        for batch in test_data_loader:
            senti_labels, topic_labels = batch['senti_labels'], batch['topic_labels']
            senti_loss, topic_loss, senti_logits, topic_logits, seq_atten, topic_atten = model(**batch)

            # loss = senti_loss + args['loss_ratio'] * topic_loss
            test_stat.add(senti_logits, senti_labels, topic_logits, topic_labels, 0)  # dummy for loss
